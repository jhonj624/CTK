    ...
    d_ptr(new ctkDICOMAppWidgetPrivate(this))
{
  ...
  QCheckBox* importCheckbox = new QCheckBox("Copiar al importar", d->ImportDialog);
  ...
  d->ImportDialog->setLabelText(QFileDialog::Accept,"Importar");
  d->ImportDialog->setWindowTitle("Importar archivos DICOM desde el directorio ...");
  d->ImportDialog->setWindowModality(Qt::ApplicationModal);
  QString dirt = QString("/home/media/");
  d->ImportDialog->setDirectory(dirt);
...
void ctkDICOMAppWidget::onImportDirectory(QString directory)
{
  ...
  if (QDir(directory).exists())
    ...
    if (d->DisplayImportSummary)
      {
      QString message = "Directorio importado completo.\n\n";
      message += QString("%1 Nuevos pacientes\n").arg(QString::number(d->PatientsAddedDuringImport));
      message += QString("%1 Nuevos estudios\n").arg(QString::number(d->StudiesAddedDuringImport));
      message += QString("%1 Nuevas series\n").arg(QString::number(d->SeriesAddedDuringImport));
      message += QString("%1 Nuevas instancias\n").arg(QString::number(d->InstancesAddedDuringImport));
      QMessageBox::information(this,"Importar Directorio DICOM", message);
      }
  }
}
